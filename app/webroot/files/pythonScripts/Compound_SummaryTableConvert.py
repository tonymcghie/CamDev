...
# Row 2 contains 'sample_reference' then the actual samples names from column 7
# Row 3 contains 'sample_description' then the actual sample description from column 7
# Row 4 contains 'crop' then the actual crop names from column 7
# Row 5 contains 'genotype' then the actual genotype from column 7
# Row 6 contains 'tissue' then the actual tissue from column 7
# Row 7 contains 'genus_species' then the actual genus_species names from column 7
# Row 8 contains 'analyst' then the actual analyst's name from column 7
# Row 9 contains 'data location' then the actual data locations from column 7
# Row 10 contains 'name' 'exact_mass' 'id_conf' 'cas' 'rt' 'intensity units' then blank from column 7
# Row 11 onwards contains the data
...
FirstCompound = 10 ## enter the row number of the first compound row
#setup the new dataframe with the necessary column
df_byCompound = pd.DataFrame(columns=['compound','exact_mass','assigned_confid','cas', 'rt_value', 'intensity_units', 'sample_ref', 'sample_description', 'crop ', 'genotype', 'tissue', 'genus_species', 'analyst', 'data_location', 'intensity_value'])
    ...
    try:
        for i,r in df.iterrows(): #for each row (compound) in a samples add data to the new datatable (df_byCompound)
            ...
            if i > FirstCompound: #the first rows of df that contain sample information and are loaded into the new df as below
                df_byCompound = df_byCompound.append({'compound':df.ix[i,0], 'exact_mass':df.ix[i,1], 'assigned_confid':df.ix[i,2], 'cas':df.ix[i,3], 'rt_value':df.ix[i,4],'intensity_units':df.ix[i,5], 'sample_ref':df.ix[0,loop],'sample_description':df.ix[1,loop], 'crop':df.ix[2,loop],'genotype':df.ix[3,loop],'tissue':df.ix[4,loop], 'genus_species':df.ix[5,loop], 'analyst':df.ix[6,loop], 'data_location':df.ix[7,loop], 'intensity_value':df.ix[i,loop]},ignore_index=True)
    except:
        break
    loop=loop+1
###########
